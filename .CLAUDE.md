...
1. **Training Backends** (`flux/training/`)
   - `base.py`: **TrainingBackend** ABC, **GPUBatch** dataclass, **TrainStepResult**
   - `backends/transformers.py`: **TransformersBackend** for HuggingFace models
   - `megatron_engine.py`: **MegatronEngine** with 3D parallelism (implements TrainingBackend)
...
3. **Adaptive Control Plane** (`flux/controller/`)
   ...
   - `mode_gate.py`: **ModeGate** state machine controlling sync/async transitions

## Key Abstractions

### Training Backend Contract
- **TrainingBackend** (`flux/training/base.py`): ABC for all training backends. GPU-direct, async-safe.
- **GPUBatch** (`flux/training/base.py`): Frozen, device-owned tensor batch. Required: `input_ids`, `attention_mask`, `behavior_log_probs`, `rewards`, `version_gaps`.
- **TrainStepResult** (`flux/training/base.py`): Standardized return type with loss, metrics, timing.
- **TransformersBackend** (`flux/training/backends/transformers.py`): HuggingFace-based backend for dev.
- **MegatronEngine** (`flux/training/megatron_engine.py`): Production backend with 3D parallelism.

### Mode Gate (Sync/Async Control)
- **AsyncMode** (`flux/controller/mode_gate.py`): Enum with `SYNC_BARRIER`, `ASYNC_RUNNING`, `THROTTLED`.
- **ModeGate** (`flux/controller/mode_gate.py`): State machine controlling sync/async transitions.
- **ModeGateIntegration**: Helper for coordinator integration.
